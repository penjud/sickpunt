                ...
                for runner_change in market_change.get('rc', []):
                   ...
                   # Initialize deques if they don't exist
                    ...
                    if not '_lay_values' in self.ff_cache[market_id][runner_id]:
                        self.ff_cache[market_id][runner_id]['_lay_values'] = deque(maxlen=1000)
                    if not '_last_values' in self.ff_cache[market_id][runner_id]:
                        self.ff_cache[market_id][runner_id]['_last_values'] = deque(maxlen=1000)
                    ...
                    self.ff_cache[market_id][runner_id]['_lay_values'].append(new_lay)
                    self.ff_cache[market_id][runner_id]['_lay_moving_avg'] = sum(self.ff_cache[market_id][runner_id]['_lay_values']) / len(self.ff_cache[market_id][runner_id]['_lay_values'])

                    new_last = runner_change.get('trd')[0][0] if runner_change.get('trd') else self.ff_cache[market_id][runner_id]['last']
                    self.ff_cache[market_id][runner_id]['_last_values'].append(new_last)
                    self.ff_cache[market_id][runner_id]['_last_moving_avg'] = sum(self.ff_cache[market_id][runner_id]['_last_values']) / len(self.ff_cache[market_id][runner_id]['_last_values'])

                    self.ff_cache[market_id][runner_id]['_back_min'] = min(self.ff_cache[market_id][runner_id]['_back_values'])
                    self.ff_cache[market_id][runner_id]['_back_max'] = max(self.ff_cache[market_id][runner_id]['_back_values'])

                    self.ff_cache[market_id][runner_id]['_lay_min'] = min(self.ff_cache[market_id][runner_id]['_lay_values'])
                    self.ff_cache[market_id][runner_id]['_lay_max'] = max(self.ff_cache[market_id][runner_id]['_lay_values'])

                    self.ff_cache[market_id][runner_id]['_last_min'] = min(self.ff_cache[market_id][runner_id]['_last_values'])
                    self.ff_cache[market_id][runner_id]['_last_max'] = max(self.ff_cache[market_id][runner_id]['_last_values'])

                    self.ff_cache[market_id][runner_id]['back'] = runner_change.get('atb')[0][0] if runner_change.get('atb') else self.ff_cache[market_id][runner_id]['back']
                    self.ff_cache[market_id][runner_id]['lay'] = runner_change.get('atl')[0][0] if runner_change.get('atl') else self.ff_cache[market_id][runner_id]['lay']
                    self.ff_cache[market_id][runner_id]['last'] = runner_change.get('trd')[0][0] if runner_change.get('trd') else self.ff_cache[market_id][runner_id]['last']
                    trd_value = runner_change.get('trd')
                    if trd_value is not None:
                        self.ff_cache[market_id][runner_id]['volume'] = trd_value[0][1] + self.ff_cache[market_id][runner_id]['last']
                    else:
                        self.ff_cache[market_id][runner_id]['volume'] = self.ff_cache[market_id][runner_id]['last']

                    self.last_cache[market_id][runner_id]['back'] = runner_change.get('atb')[0][0] if runner_change.get('atb') else None
